...
class Almanac:
  def __init__(self, text: str) -> None:
    self.seeds = () # Generator
    self.seed_locations = () # Generator
    self.seed_to_soil = {}
  ...
  def populate_seeds(self, line: str) -> None:
    _, seeds_data = line.split(':')
    seeds_data = seeds_data.strip().split()
    seeds_data = zip(seeds_data[::2], seeds_data[1::2])
    self.seeds = (int(seed) for seed_start, seed_range in seeds_data for seed in range(int(seed_start), int(seed_start) + int(seed_range)))
